...
const EmptiesDispatchSearchPanel = ({ filters, onFilterChange, onSearch, onClose, vendors, locations }) => {
    ...
    <div className="absolute top-full left-0 mt-1 z-30 bg-card border border-border rounded-lg shadow-xl p-4 w-96">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input
            className={inputCls}
            placeholder="e.g. EDS-2026-03-08-001"
            value={filters?.dispatch_no || ''}
            onChange={e => onFilterChange('dispatch_no', e?.target?.value)}
          />
        </div>
        <div>
          <label className={labelCls}>Supplier</label>
          <select className={inputCls} value={filters?.supplier_id || ''} onChange={e => onFilterChange('supplier_id', e?.target?.value)}>
...
export default EmptiesDispatchSearchPanel;
