...
const EmptiesDispatchSpreadsheet = ({ rows, isLoading, selectedIds, onSelectAll, onSelectRow, onRowDoubleClick, sortConfig, onSort }) => {
  const SortIcon = ({ col }) => {
  ...
  const columns = [
    { key: 'dispatch_no', label: 'Dispatch No', w: 'w-36' },
    { key: 'dispatch_date', label: 'Dispatch Date', w: 'w-28' },
    { key: 'supplier_name', label: 'Supplier Name', w: 'w-48' },
        ...
        <thead className="sticky top-0 z-10">
          <tr className="bg-primary">
            <th className="px-2 py-2 text-center w-8">
              <input type="checkbox" checked={allSelected} onChange={onSelectAll} className="w-3 h-3 rounded" />
            </th>
          ...
          {isLoading ? (
            ...
            <tr>
              <td colSpan={9} className="px-3 py-12 text-center text-xs text-muted-foreground">
                No empties dispatches found. Press New (F2) to create one.
              </td>
            </tr>
          ) : (
            rows?.map((row, idx) => {
              ...
              return (
                ...
                >
                  <td className="px-2 py-1.5 text-center">
                    <input
                      type="checkbox" checked={isSelected} onChange={() => onSelectRow(row?.id)}
                      className="w-3 h-3 rounded" onClick={e => e?.stopPropagation()}
                    />
                  </td>
                  <td className="px-2 py-1.5 text-center text-xs text-muted-foreground">{idx + 1}</td>
                  <td className={tdCls}><span className="font-mono text-primary text-xs">{row?.dispatch_no}</span></td>
                  <td className={tdCls}>{fmtDate(row?.dispatch_date)}</td>
                  <td className={tdCls + ' max-w-[192px] truncate'}>{row?.supplier_name || '—'}</td>
...
export default EmptiesDispatchSpreadsheet;
