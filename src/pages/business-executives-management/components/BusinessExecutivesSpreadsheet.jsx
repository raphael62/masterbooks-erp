...
import BusinessExecutiveModal from './BusinessExecutiveModal';

const ITEMS_PER_PAGE_OPTIONS = [10, 20, 50, 100];
const COLUMNS = [
  { key: 'exec_code', label: 'Executive Code', width: 'w-32' },
  { key: 'full_name', label: 'Full Name', width: 'w-44' },
  ...
  { key: 'sales_rep_type', label: 'Rep Type', width: 'w-24' },
  { key: 'target_amount', label: 'Target Amount', width: 'w-32' },
  { key: 'sales_ytd', label: 'Sales YTD', width: 'w-32' },
  { key: 'status', label: 'Status', width: 'w-24' },
];

const BusinessExecutivesSpreadsheet = () => {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [execRes, compRes] = await Promise.all([
        supabase?.from('business_executives')?.select('*, companies(name)')?.order('exec_code'),
        supabase?.from('companies')?.select('id, name')?.order('name'),
      ]);
      if (execRes?.error) throw execRes?.error;
      const enriched = (execRes?.data || [])?.map(e => ({
        ...e,
        company_name: e?.companies?.name || '',
      }));
      setExecutives(enriched);
  ...
  const uniqueLocations = useMemo(() => {
    const locs = executives?.map(e => e?.location)?.filter(Boolean);
    return [...new Set(locs)]?.sort();
  }, [executives]);

  const filteredData = useMemo(() => {
    let data = [...executives];
    if (filterCode) data = data?.filter(r => r?.exec_code?.toLowerCase()?.includes(filterCode?.toLowerCase()));
    if (filterName) data = data?.filter(r => r?.full_name?.toLowerCase()?.includes(filterName?.toLowerCase()));
    if (filterCompany) data = data?.filter(r => r?.company_id === filterCompany);
    if (filterLocation) data = data?.filter(r => r?.location?.toLowerCase()?.includes(filterLocation?.toLowerCase()));
    ...
    return data;
  }, [executives, filterCode, filterName, filterCompany, filterLocation, filterStatus, sortConfig]);

  const totalPages = Math.max(1, Math.ceil(filteredData?.length / itemsPerPage));
  const paginatedData = filteredData?.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  const totalTarget = filteredData?.reduce((sum, r) => sum + (Number(r?.target_amount) || 0), 0);
  const totalSalesYTD = filteredData?.reduce((sum, r) => sum + (Number(r?.sales_ytd) || 0), 0);
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase?.from('business_executives')?.delete()?.in('id', toDelete);
  ...
  const handleEdit = () => {
    ...
    if (!id) return;
    const item = executives?.find(r => r?.id === id);
  ...
  const handleExcel = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'business_executives.csv'; a?.click();
    ...
    <div className="flex flex-col h-full bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-foreground">Business Executives</span>
        ...
        <div className="relative" ref={filterPanelRef}>
          ...
          {showFilterPanel && (
            <div className="absolute right-0 top-full mt-1 w-80 bg-card border border-border rounded-lg shadow-lg z-50 p-4">
              <h4 className="text-sm font-semibold text-foreground mb-3">Filter Executives</h4>
              <div className="space-y-3">
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Executive Code</label>
                ...
                <div>
                  ...
                  <input type="text" value={filterLocation} onChange={e => { setFilterLocation(e?.target?.value); setCurrentPage(1); }}
                ...
                <button onClick={() => { setFilterCode(''); setFilterName(''); setFilterCompany(''); setFilterLocation(''); setFilterStatus('all'); setCurrentPage(1); }}
        ...
        <table className="w-full text-xs font-sans border-collapse">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={COLUMNS?.length + 2} className="text-center py-12 text-muted-foreground">No executives found</td></tr>
            ) : (
              paginatedData?.map((row, idx) => {
                ...
                return (
                  <tr key={row?.id}
                    ...
                    <td className="px-3 py-1.5 border-r border-border font-mono font-medium text-primary tabular-nums">{row?.exec_code}</td>
                    <td className="px-3 py-1.5 border-r border-border font-medium text-foreground">{row?.full_name}</td>
                    <td className="px-3 py-1.5 border-r border-border text-muted-foreground tabular-nums">{row?.phone || '—'}</td>
                    <td className="px-3 py-1.5 border-r border-border text-muted-foreground truncate max-w-xs">{row?.email || '—'}</td>
                    <td className="px-3 py-1.5 border-r border-border text-muted-foreground">{row?.company_name || '—'}</td>
                    <td className="px-3 py-1.5 border-r border-border text-muted-foreground">{row?.location || '—'}</td>
                    <td className="px-3 py-1.5 border-r border-border">
                      {row?.sales_rep_type ? (
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
                          row?.sales_rep_type === 'VSR' ?'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400'
                        }`}>{row?.sales_rep_type}</span>
                      ) : <span className="text-muted-foreground">—</span>}
                    </td>
                    <td className="px-3 py-1.5 border-r border-border tabular-nums text-right">{fmt(row?.target_amount)}</td>
                    <td className="px-3 py-1.5 border-r border-border tabular-nums text-right">
                      <span className={Number(row?.sales_ytd) >= Number(row?.target_amount) && Number(row?.target_amount) > 0 ? 'text-green-600 dark:text-green-400 font-medium' : ''}>
                        {fmt(row?.sales_ytd)}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 border-r border-border">
            ...
            {!isLoading && filteredData?.length > 0 && (
              <tr className="bg-muted/50 border-t-2 border-border font-semibold">
                <td className="px-2 py-2 text-center border-r border-border" colSpan={2}></td>
                <td className="px-3 py-2 border-r border-border text-foreground" colSpan={7}>Grand Total: {filteredData?.length} executives</td>
                <td className="px-3 py-2 border-r border-border tabular-nums text-right text-foreground">{fmt(totalTarget)}</td>
                <td className="px-3 py-2 border-r border-border tabular-nums text-right text-foreground">{fmt(totalSalesYTD)}</td>
...
export default BusinessExecutivesSpreadsheet;
