...
const OpenMarketModal = ({ isOpen, onClose, onSaved, editRecord }) => {
  ...
  const generatePurchaseNo = useCallback(async () => {
    const dateStr = today();
    const prefix = `OMP-${dateStr}`;
    const { data } = await supabase
      ?.from('empties_open_market_header')
      ?.select('purchase_no')
      ?.ilike('purchase_no', `${prefix}%`)
      ?.order('purchase_no', { ascending: false })
      ?.limit(1);
    const last = data?.[0]?.purchase_no;
    let seq = 1;
    if (last) {
  ...
  useEffect(() => {
    ...
    if (isEdit && editRecord) {
      setHeader({
        purchase_no: editRecord?.purchase_no || '',
        purchase_date: editRecord?.purchase_date || today(),
        supplier_name: editRecord?.supplier_name || '',
        location_id: editRecord?.location_id || '',
        location_name: editRecord?.location_name || '',
        notes: editRecord?.notes || '',
      });
      setLines(editRecord?.items?.length > 0
        ? editRecord?.items?.map(it => ({ ...it, _key: it?.id || Math.random()?.toString(36)?.slice(2) }))
        : [emptyLine()]);
    } else {
      generatePurchaseNo()?.then(no => setHeader(h => ({ ...h, purchase_no: no })));
      setHeader(h => ({ ...h, purchase_date: today(), supplier_name: '', location_id: '', location_name: '', notes: '' }));
      setLines([emptyLine()]);
    }
    setError('');
  }, [isOpen, isEdit, editRecord, fetchLookups, generatePurchaseNo]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        const { error: updErr } = await supabase?.from('empties_open_market_header')?.update({
        ...
        await supabase?.from('empties_open_market_items')?.delete()?.eq('header_id', headerId);
      } else {
        const { data: hData, error: hErr } = await supabase?.from('empties_open_market_header')?.insert({
          purchase_no: header?.purchase_no,
          purchase_date: header?.purchase_date,
      ...
      const { error: itemErr } = await supabase?.from('empties_open_market_items')?.insert(itemsToInsert);
      if (itemErr) throw itemErr;

      // STOCK ONLY: Increase stock_levels_by_location (NO empties_ledger entry)
      ...
      <div className="bg-card rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col border border-border">
        <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-primary rounded-t-xl">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-primary-foreground">
              {isEdit ? 'Edit Open Market Purchase' : 'New Open Market Purchase'}
            </h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div>
              <label className={labelCls}>Purchase No</label>
              <input className={inputCls + ' bg-muted/50 font-mono'} value={header?.purchase_no} readOnly />
            </div>
            <div>
              <label className={labelCls}>Purchase Date *</label>
              <input type="date" className={inputCls} value={header?.purchase_date} onChange={e => handleHeaderChange('purchase_date', e?.target?.value)} />
            </div>
            <div>
              <label className={labelCls}>Supplier Name (Free Text)</label>
              <input
                className={inputCls}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-border bg-muted/30 rounded-b-xl">
          <div className="flex items-center gap-1.5">
            ...
            <p className="text-xs text-muted-foreground">On save: stock increases at location only — no empties ledger obligation</p>
          </div>
