...
const OpenMarketSearchPanel = ({ filters, onFilterChange, onSearch, onClose, locations }) => {
  ...
  return (
    <div className="absolute top-full right-0 mt-1 z-30 bg-card border border-border rounded-lg shadow-xl p-4 w-96">
      ...
      <div className="space-y-3">
        <div>
          ...
          <input
            className={inputCls}
            placeholder="e.g. OMP-2026-03-08-001"
            value={filters?.purchase_no || ''}
            onChange={e => onFilterChange('purchase_no', e?.target?.value)}
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={e => onFilterChange('supplier_name', e?.target?.value)}
          />
        </div>
        <div>
...
export default OpenMarketSearchPanel;
