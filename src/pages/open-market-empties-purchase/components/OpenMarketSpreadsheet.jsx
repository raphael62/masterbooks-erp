...
const OpenMarketSpreadsheet = ({ rows, isLoading, selectedIds, onSelectAll, onSelectRow, onRowDoubleClick, sortConfig, onSort }) => {
  const SortIcon = ({ col }) => {
  ...
  const columns = [
    { key: 'purchase_no', label: 'Purchase No', w: 'w-40' },
    { key: 'purchase_date', label: 'Purchase Date', w: 'w-28' },
          ...
          {isLoading ? (
            ...
            <tr>
              <td colSpan={9} className="px-3 py-12 text-center text-xs text-muted-foreground">
                No open market purchases found. Press New (F2) to create one.
              </td>
            </tr>
          ) : (
            rows?.map((row, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={tdCls}><span className="font-mono text-primary text-xs">{row?.purchase_no}</span></td>
                  <td className={tdCls}>{fmtDate(row?.purchase_date)}</td>
...
export default OpenMarketSpreadsheet;
