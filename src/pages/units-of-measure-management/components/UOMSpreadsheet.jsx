...
const COLUMNS = [
  { key: 'uom_code', label: 'UOM Code', width: 'w-28' },
  { key: 'uom_name', label: 'UOM Name', width: 'w-40' },
  { key: 'uom_type', label: 'Type', width: 'w-28' },
...
const UOMSpreadsheet = () => {
  ...
  const fetchUOMs = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase?.from('units_of_measure')?.select('*')?.order('uom_code');
  ...
  const filteredData = useMemo(() => {
    let data = [...uoms];
    if (filterCode) data = data?.filter(r => r?.uom_code?.toLowerCase()?.includes(filterCode?.toLowerCase()));
    if (filterName) data = data?.filter(r => r?.uom_name?.toLowerCase()?.includes(filterName?.toLowerCase()));
    if (filterType !== 'all') data = data?.filter(r => r?.uom_type === filterType);
    ...
    return data;
  }, [uoms, filterCode, filterName, filterType, filterStatus, sortConfig]);
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase?.from('units_of_measure')?.delete()?.in('id', toDelete);
      if (error) throw error;
  ...
  const handleEdit = () => {
    ...
    if (!id) return;
    const item = uoms?.find(r => r?.id === id);
  ...
  const handleExcel = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'units_of_measure.csv'; a?.click();
    ...
    <div className="flex flex-col h-full bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-foreground">Units of Measure</span>
          ...
          <div className="relative" ref={filterPanelRef}>
            ...
            {showFilterPanel && (
              <div className="absolute right-0 top-full mt-1 w-64 bg-card border border-border rounded-lg shadow-lg z-50 p-4">
                <h4 className="text-sm font-semibold text-foreground mb-3">Filter UOMs</h4>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">UOM Code</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">UOM Name</label>
        ...
        <table className="w-full text-xs font-sans border-collapse">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={COLUMNS?.length + 2} className="text-center py-12 text-muted-foreground">No units of measure found</td></tr>
            ) : (
              paginatedData?.map((row, idx) => {
                ...
                return (
                  <tr key={row?.id} onClick={() => setSelectedRow(row?.id)}
                    ...
                    <td className="px-3 py-1.5 border-r border-border font-mono font-medium text-foreground">{row?.uom_code}</td>
                    <td className="px-3 py-1.5 border-r border-border font-medium text-foreground">{row?.uom_name}</td>
                    <td className="px-3 py-1.5 border-r border-border">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                        TYPE_COLORS?.[row?.uom_type] || 'bg-gray-100 text-gray-600'
                      }`}>{row?.uom_type}</span>
                    </td>
                    <td className="px-3 py-1.5 border-r border-border">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
          ...
          {filteredData?.length > 0 && (
            <tfoot>
              <tr className="bg-muted/50 border-t-2 border-border font-semibold">
                <td colSpan={2} className="px-3 py-2 text-xs text-muted-foreground border-r border-border"></td>
                <td colSpan={2} className="px-3 py-2 text-xs font-bold text-foreground border-r border-border">Grand Total: {filteredData?.length} records</td>
      ...
      {showModal && (
        <UOMModal
          ...
          onSaved={() => { setShowModal(false); setEditItem(null); fetchUOMs(); }}
        />
      )}
    </div>
  );
