...
const COLUMNS = [
  { key: 'tax_code', label: 'Tax Code', width: 'w-28' },
  { key: 'tax_name', label: 'Tax Name', width: 'w-48' },
  { key: 'tax_type', label: 'Tax Type', width: 'w-40' },
  { key: 'rate', label: 'Rate %', width: 'w-24' },
  { key: 'applies_to', label: 'Applies To', width: 'w-28' },
  { key: 'effective_date', label: 'Effective Date', width: 'w-32' },
  { key: 'status', label: 'Status', width: 'w-24' },
];

const TAX_TYPE_COLORS = {
  'VAT': 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
  'Withholding Tax': 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
  'NHIL': 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400',
  'GetFund Levy': 'bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400',
  'COVID Levy': 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
};

const TaxRatesSpreadsheet = () => {
  ...
  const fetchTaxRates = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data, error } = await supabase?.from('tax_rates')?.select('*')?.order('tax_code');
  ...
  const filteredData = useMemo(() => {
    let data = [...taxRates];
    if (filterCode) data = data?.filter(r => r?.tax_code?.toLowerCase()?.includes(filterCode?.toLowerCase()));
    if (filterName) data = data?.filter(r => r?.tax_name?.toLowerCase()?.includes(filterName?.toLowerCase()));
    if (filterType !== 'all') data = data?.filter(r => r?.tax_type === filterType);
    ...
    return data;
  }, [taxRates, filterCode, filterName, filterType, filterStatus, sortConfig]);

  const totalPages = Math.max(1, Math.ceil(filteredData?.length / itemsPerPage));
  const paginatedData = filteredData?.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  const grandTotalRate = filteredData?.reduce((s, r) => s + Number(r?.rate || 0), 0);
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase?.from('tax_rates')?.delete()?.in('id', toDelete);
      if (error) throw error;
  ...
  const handleEdit = () => {
    ...
    if (!id) return;
    const item = taxRates?.find(r => r?.id === id);
  ...
  const handleExcel = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'tax_rates.csv'; a?.click();
    ...
    <div className="flex flex-col h-full bg-card border border-border rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-muted/30 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-foreground">Tax & VAT Rates</span>
          ...
          <div className="relative" ref={filterPanelRef}>
            ...
            {showFilterPanel && (
              <div className="absolute right-0 top-full mt-1 w-72 bg-card border border-border rounded-lg shadow-lg z-50 p-4">
                <h4 className="text-sm font-semibold text-foreground mb-3">Filter Tax Rates</h4>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">Tax Code</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-muted-foreground">Tax Name</label>
                  ...
                  <div>
                    ...
                    <select value={filterType} onChange={e => { setFilterType(e?.target?.value); setCurrentPage(1); }}
                      ...
                      <option value="GetFund Levy">GetFund Levy</option>
        ...
        <table className="w-full text-xs font-sans border-collapse">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={COLUMNS?.length + 2} className="text-center py-12 text-muted-foreground">No tax rates found</td></tr>
            ) : (
              paginatedData?.map((row, idx) => {
                ...
                return (
                  <tr key={row?.id} onClick={() => setSelectedRow(row?.id)}
                    ...
                    <td className="px-3 py-1.5 border-r border-border font-mono font-medium text-primary">{row?.tax_code}</td>
                    <td className="px-3 py-1.5 border-r border-border font-medium text-foreground">{row?.tax_name}</td>
                    <td className="px-3 py-1.5 border-r border-border">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        TAX_TYPE_COLORS?.[row?.tax_type] || 'bg-gray-100 text-gray-600'
                      }`}>{row?.tax_type}</span>
                    </td>
                    <td className="px-3 py-1.5 border-r border-border tabular-nums text-right font-semibold">{Number(row?.rate || 0)?.toFixed(2)}%</td>
                    <td className="px-3 py-1.5 border-r border-border">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        row?.applies_to === 'Both' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' :
                        row?.applies_to === 'Sales'? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400'
                      }`}>{row?.applies_to}</span>
                    </td>
                    <td className="px-3 py-1.5 border-r border-border text-muted-foreground">{row?.effective_date || '—'}</td>
                    <td className="px-3 py-1.5 border-r border-border">
          ...
          {filteredData?.length > 0 && (
            <tfoot className="sticky bottom-0 z-10">
              <tr className="bg-muted/80 border-t-2 border-border font-semibold">
                <td colSpan={4} className="px-3 py-2 text-right text-xs text-foreground border-r border-border">Grand Total</td>
                <td className="px-3 py-2 tabular-nums text-right text-xs text-primary font-bold border-r border-border">{grandTotalRate?.toFixed(2)}%</td>
                <td colSpan={3} className="px-3 py-2 border-r border-border"></td>
              </tr>
            </tfoot>
          )}
        </table>
      ...
      {showModal && (
        <TaxRateModal key={editItem?.id ?? 'new'} item={editItem} onClose={() => { setShowModal(false); setEditItem(null); }} onSaved={() => { setShowModal(false); setEditItem(null); fetchTaxRates(); }} />
      )}
    </div>
  );
};

export default TaxRatesSpreadsheet;
