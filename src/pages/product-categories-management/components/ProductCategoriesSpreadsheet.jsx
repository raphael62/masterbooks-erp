...
const COLUMNS = [
  ...
  { key: 'product_count', label: 'Product Count', width: 'w-32' },
  { key: 'status', label: 'Status', width: 'w-24' },
];

const ProductCategoriesSpreadsheet = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data;
  }, [categories, filterCode, filterName, filterStatus, sortConfig]);

  const totalPages = Math.max(1, Math.ceil(filteredData?.length / pageSize));
  const paginatedData = filteredData?.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  const grandTotalCount = filteredData?.reduce((s, r) => s + Number(r?.product_count || 0), 0);
    ...
    <div className="flex flex-col h-full bg-card border border-border rounded-lg overflow-hidden">
              ...
              paginatedData?.map((row, idx) => {
                ...
                return (
                  <tr key={row?.id} onClick={() => setSelectedRow(row?.id)}
                    ...
                    <td className="px-3 py-1.5 border-r border-border font-mono font-medium text-foreground">{row?.category_code}</td>
                    ...
                    <td className="px-3 py-1.5 border-r border-border tabular-nums text-right">{row?.product_count ?? 0}</td>
          ...
          {filteredData?.length > 0 && (
            <tfoot>
              <tr className="bg-muted/50 border-t-2 border-border font-semibold">
                ...
                <td colSpan={2} className="px-3 py-2 text-xs font-bold text-foreground border-r border-border">Grand Total</td>
                <td className="px-3 py-2 border-r border-border"></td>
                <td className="px-3 py-2 text-right tabular-nums font-bold text-foreground border-r border-border">{grandTotalCount}</td>
                <td className="px-3 py-2 border-r border-border"></td>
              </tr>
            </tfoot>
          )}
      ...
      {showModal && (
        <ProductCategoryModal
          ...
          onSaved={() => { setShowModal(false); setEditItem(null); fetchCategories(); }}
        />
      )}
    </div>
  );
...
export default ProductCategoriesSpreadsheet;
